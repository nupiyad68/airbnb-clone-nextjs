	...
	return (
		<header className="sticky top-0 z-50 grid grid-cols-3 bg-white shadow-md p-5 md:px-10">
			...
			<div className="flex items-center md:border-2 rounded-full py-2 md:shadow-sm">
				<input 
					className="overflow-hidden pl-5 bg-transparent outline-none text-sm text-gray-600 flex-grow"
					type="text"
					placeholder={placeholder || "Start Your Search"}
					value={searchInput}
					onChange={(e) => setSearchInput(e.target.value)}
				/>
				<SearchIcon className="hidden md:inline-flex h-8 bg-red-400 text-white rounded-full p-2 cursor-pointer md:mx-2 right-0" />
			</div>
